...
def list_glucose_logs(user_id: str, limit: int = Query(50, ge=1, le=200), days: Optional[int] = Query(None), current_user=Depends(get_current_user)):
    ...
    try:
        try:
            from src.features.anomaly_detector import SPIKE_HIGH, SPIKE_LOW
        except ImportError:
            from features.anomaly_detector import SPIKE_HIGH, SPIKE_LOW  # type: ignore
        for _l in logs:
            try:
...
@app.post(f"{API_PREFIX}/glucose/followup", tags=["glucose"])
...
def list_doctor_patients(current_user=Depends(require_expert)):
    ...
    try:
        ...
        from features.base_tracker import get_conn as _get_conn  # type: ignore
    try:
        from src.features.anomaly_detector import detect_spike, detect_trend, SPIKE_HIGH, SPIKE_LOW
    except ImportError:
        from features.anomaly_detector import detect_spike, detect_trend, SPIKE_HIGH, SPIKE_LOW  # type: ignore
    try:
