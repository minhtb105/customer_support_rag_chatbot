...
import time
try:
    from shared.config import META_DB_PATH
except ImportError:
    from src.shared.config import META_DB_PATH
...
def get_chunk_hashes_for_file(file_name, db_path=META_DB_PATH):
    with sqlite3.connect(db_path) as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT chunk_hash FROM chunks WHERE file_name=? ORDER BY chunk_index", (file_name,))
        rows = cur.fetchall()
...
def find_vector_ids_for_chunk_hashes(chunk_hashes, db_path=META_DB_PATH):
    ...
    q = "SELECT vector_id FROM chunks WHERE chunk_hash IN ({})".format(
