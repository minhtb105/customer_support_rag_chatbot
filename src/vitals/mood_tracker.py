...
Stores PHQ-9, GAD-7, crisis flags via BaseTracker.
"""
...
try:
    from src.shared.config import MENTAL_HEALTH_THRESHOLDS, VITALS_DB_PATH, PII_REDACT_FIELDS
    from src.vitals.base_tracker import get_conn as _base_get_conn, init_table, fetch_logs, classification_counts
except ImportError:  # pragma: no cover
    from shared.config import MENTAL_HEALTH_THRESHOLDS, VITALS_DB_PATH, PII_REDACT_FIELDS  # type: ignore
    from vitals.base_tracker import get_conn as _base_get_conn, init_table, fetch_logs, classification_counts  # type: ignore
...
def init_mood_db():
    init_table(
        ...
        """,
        "CREATE INDEX IF NOT EXISTS idx_mood_user_time ON mood_logs(user_id, measured_at)",
    )
...
def get_mood_logs(user_id: str, limit: int = 50, days: Optional[int] = None) -> List[Dict[str, Any]]:
    init_mood_db()
    return fetch_logs(VITALS_DB_PATH, "mood_logs", user_id, limit, days)
