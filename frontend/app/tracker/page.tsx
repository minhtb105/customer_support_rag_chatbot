...
function classColor(c: string) {
  ...
  if (c === "elevated") return "bg-amber-100 text-amber-700 border-amber-200";
  if (c === "high") return "bg-red-100 text-red-700 border-red-200";
  if (c === "critical") return "bg-red-600 text-white border-red-700";
  if (c === "low") return "bg-orange-100 text-orange-700 border-orange-200";
...
export default function TrackerPage() {
  ...
  const refresh = async () => {
    try {
      const d = await getGlucose(userId, 50);
      setData(d);
    } catch (e: any) {
      setMsg(e.message);
    }
  ...
  const submit = async () => {
    ...
    setTrend("");
    try {
      const rec: any = await logGlucose({ user_id: userId, value_mgdl: Number(value), context, notes: notes || undefined });
  ...
  const explainTrend = async () => {
    ...
    try {
      const ans = await queryRag(q, 5, userId);
      setTrend(ans.answer);
    } catch (e: any) {
      setTrend("Lỗi RAG: " + e.message + " (kiểm tra FastAPI đã chạy và đã index PDFs)");
    }
  };

  const chartData = data?.logs ? [...data.logs].reverse().slice(-20).map((l: any) => ({
    time: l.measured_at.slice(5, 16).replace("T", " "),
    value: l.value_mgdl,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold">Hướng A — Nhật ký đường huyết</h1>
        <p className="text-sm text-slate-600">Nhắm 20,2% tự theo dõi. Mục tiêu KPI: ≥3 lần/tuần sau 4 tuần. Chỉ escalate khi critical hoặc 3×high liên tiếp.</p>
      </div>
        ...
        <div className="rounded-2xl border bg-white p-5 shadow-sm lg:col-span-1">
          ...
          <div className="mt-4 space-y-3">
            ...
            <div className="rounded-lg bg-blue-50 p-3 text-xs text-blue-800 border border-blue-100">
              Ngưỡng WHO/ADA: Đói bình thường &lt;100, tiền ĐTĐ 100–125, ĐTĐ ≥126 · Sau ăn 2h bình thường &lt;140, tiền 140–199, ĐTĐ ≥200 · Hạ &lt;70 · Critical ≥300
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6 lg:col-span-2">
          {data?.stats && (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <Kpi label="Tần suất" value={`${data.stats.logs_per_week}/tuần`} sub={data.stats.logs_per_week < 3 ? "Chưa đạt KPI 3/tuần" : "Đạt KPI"} alert={data.stats.logs_per_week < 3} />
            </div>
          )}

          <div className="rounded-2xl border bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold">Biểu đồ 20 lần gần nhất</h3>
              <button onClick={explainTrend} className="rounded-full bg-slate-900 px-4 py-2 text-xs text-white hover:bg-black">Diễn giải xu hướng (RAG)</button>
            </div>
              ...
              {chartData.length ? (
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData}>
                    ...
                    <Tooltip />
                    <ReferenceLine y={126} stroke="#f59e0b" strokeDasharray="4 4" label={{ value: "126 (ĐTĐ đói)", fontSize: 10 }} />
                    <ReferenceLine y={200} stroke="#ef4444" strokeDasharray="4 4" label={{ value: "200 (sau ăn)", fontSize: 10 }} />
                    <ReferenceLine y={70} stroke="#f97316" strokeDasharray="4 4" />
                    <Line type="monotone" dataKey="value" stroke="#0ea5e9" strokeWidth={2} dot={{ r: 3 }} />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
                <div className="flex h-full items-center justify-center text-sm text-slate-500">Chưa có dữ liệu — hãy nhập lần đo đầu tiên.</div>
              )}
            </div>
            {data?.should_escalate && <div className="mt-3 rounded-lg bg-red-50 border border-red-200 p-3 text-xs text-red-800">⚠️ Khuyến nghị liên hệ bác sĩ: vượt ngưỡng critical hoặc 3 lần high liên tiếp.</div>}
            ...
            <div className="mt-3 max-h-72 overflow-auto divide-y rounded-lg border">
              {data?.logs?.length ? data.logs.map((l: any) => (
                <div key={l.id} className="flex items-center justify-between px-3 py-2 text-xs">
                  <div>
                    <div className="font-mono">{l.measured_at.slice(0, 16).replace("T", " ")} · {l.value_mgdl} mg/dL</div>
                    <div className="text-slate-500">{l.context} {l.notes ? `· ${l.notes}` : ""}</div>
                  </div>
                  <span className={`rounded-full border px-2 py-1 text-[11px] font-medium ${classColor(l.classification)}`}>{l.classification}</span>
                </div>
...
function Kpi({ label, value, sub, alert }: any) {
  return (
    <div className={`rounded-xl border p-3 ${alert ? "bg-amber-50 border-amber-200" : "bg-white"}`}>
      <div className="text-[11px] text-slate-500">{label}</div>
      <div className="text-sm font-bold">{value}</div>
      {sub && <div className={`text-[11px] ${alert ? "text-amber-700" : "text-slate-500"}`}>{sub}</div>}
    </div>
  );
}
